...
public class HobbyistService {
    ...
    public Optional<User> getHobbyistById(Long id) {
        return hobbyistRepository.findById(id);
    }

    public List<User> getAllHobbyists() {
    ...
    public Hobbyist updateHobbyist(Long id, Hobbyist updatedHobbyist) {
        Optional<User> existingHobbyist = hobbyistRepository.findById(id);
        if (existingHobbyist.isPresent()) {
            Hobbyist hobbyist = (Hobbyist) existingHobbyist.get();
